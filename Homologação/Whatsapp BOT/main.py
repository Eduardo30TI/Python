...
def Main():
    ...
    try:
        ...
        menu=['Configurar','Script','Iniciar']
...
def Whatsapp():
    ...
    while True:
        ...
        for i,script in enumerate(scripts):
            ...
            dir_name=gui.GetArq(gui.PathDir(script))

            print(f'Executando o script {dir_name}. Aguarde...')
...
def Script():
    ...
    temp_dict={'Caminho':'','Arquivo':''}

    while True:

        for tipo in temp_dict.keys():

            val=input(f'Informe o {tipo}: ')

            temp_dict[tipo]=val

            pass

        caminho=gui.UnirPath(temp_dict['Caminho'],temp_dict['Arquivo'])

        if gui.GetExists(caminho)==True:

            break

        pass

    gui.ExecutarScript(caminho)
