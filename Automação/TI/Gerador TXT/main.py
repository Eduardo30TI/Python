...
querys={
    ...
    WITH TabDatas(dt_criacao,cd_fila) AS (
    SELECT DISTINCT CONVERT(DATETIME,CAST(dt_criacao AS DATE),101) AS dt_criacao,cd_fila
    FROM evento
    WHERE cd_fila IN('FATU','CAPV') AND cd_emp=1),

    TabDias (dt_criacao) AS (

    SELECT DISTINCT * FROM (
    SELECT dt_criacao
    FROM TabDatas
    WHERE (SELECT COUNT(Tab01.dt_criacao) FROM TabDatas AS Tab01 WHERE Tab01.dt_criacao=TabDatas.dt_criacao)>1 AND cd_fila='FATU'
    UNION ALL
    SELECT dt_criacao
    FROM TabDatas
    WHERE (SELECT COUNT(Tab01.dt_criacao) FROM TabDatas AS Tab01 WHERE Tab01.dt_criacao=TabDatas.dt_criacao)<=1)Tab02)

    SELECT dt_criacao AS 'Data Trabalhado', CASE WHEN DATEPART(WEEKDAY,dt_criacao) IN (1,7) THEN 0 ELSE 1 END AS 'Dias Úteis'
    FROM TabDias
    ORDER BY 1    
    ...
    DECLARE @BASE DATETIME, @DTFIM DATETIME,@DTINICIO DATETIME

    SET @BASE=CONVERT(DATETIME,CAST(GETDATE() AS DATE),101)

    SET @DTFIM=DATEADD(DAY,DAY(@BASE)*-1,@BASE)

    SET @DTINICIO=CONCAT(YEAR(GETDATE())-1,'-01-','01')

    SELECT * FROM netfeira.vw_targetestatico
    WHERE [Data de Faturamento] BETWEEN @DTINICIO AND @DTFIM AND Situação<>'EM ABERTO'
...
if __name__=='__main__':
    ...
    temp_path=os.path.join(os.getcwd(),'Arquivos TXT')
    
    for arq,tabela in tabelas_df.items():

        temp_df=tabela

        base_path=os.path.join(temp_path,f'{arq}.txt')

        temp_df.to_csv(base_path,index=False,encoding='UTF-8')
